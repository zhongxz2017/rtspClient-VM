...
//  the length of encoded is longer than before, about 1/3 memory space, so make sure there is enough memory space for base64code 
...
// Get single byte from two bytes 
#define B0(a) (a & 0xFF)
...
{   
	...
	// processing the less than three bytes data 
	if (i < src_len)
	{
		int rest = src_len - i;
...
//  the length of decode is less than before, about 1/4 memory space, so make sure there is enough memory space for buf
...
{   
	...
	// processing the less than 4 bytes data
	if (i < src_len)
	{
		int rest = src_len - i;
		unsigned long ulTmp = 0;
